...
function getAgencyColor(agency) {
  if (!agency) return agencyColors.Other;
...
function getDaysUntil(dateStr) {
  if (!dateStr || !/^\d{4}-\d{2}-\d{2}$/.test(String(dateStr))) return 9999;
...
function urgencyLabel(days) {
  if (days >= 9999) return { label:"No date", color:"#888", bg:"#2a2a2a" };
...
export default function GrantTracker() {
  ...
  useEffect(() => {
    if (!SCRIPT_URL) {
      try {
        ...
        setGrants(raw ? JSON.parse(raw) : sampleGrants);
      ...
      .then(text => {
        ...
        if (data.success && Array.isArray(data.grants) && data.grants.length > 0) {
          const clean = data.grants
            ...
            .map(g => ({ ...g, deadline: normalizeDate(g.deadline), category: g.category || "" }));
  ...
  const handleSave = () => {
    ...
    setGrants(updated); saveToSheet(updated);
  ...
  const usedCats      = ["All", ...CATEGORIES.filter(c => grants.some(g => g.category === c))];
  const urgentCount   = grants.filter(g => { const d=getDaysUntil(g.deadline); return d>=0&&d<=14; }).length;

  const filtered = grants
    .filter(g => { const d=getDaysUntil(g.deadline); return d>=-7 && new Date(g.deadline+"T00:00:00")<=windowEnd; })
  ...
  // Styles ---------------------------------------------------------------
  const sel = { background:"#0d1f35", border:"1px solid #1e3a5f", borderRadius:7, color:"#c9d8ed", padding:"7px 14px", fontSize:13, fontFamily:"inherit", outline:"none" };
  const inp = (x={}) => ({ background:"#0d1828", border:"1px solid #1e3a5f", borderRadius:8, color:"#c9d8ed", padding:"9px 12px", fontSize:13, fontFamily:"inherit", outline:"none", width:"100%", boxSizing:"border-box", colorScheme:"dark", ...x });
  const btn = (bg,br,tc) => ({ background:bg, border:"1px solid "+br, borderRadius:8, color:tc, padding:"9px 22px", cursor:"pointer", fontFamily:"inherit", fontSize:14 });
  const lbl = { fontSize:11, color:"#4a7a9b", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:5 };
        ...
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
            ...
            <div style={{ display:"flex", gap:12, alignItems:"center", flexWrap:"wrap" }}>
              {urgentCount > 0 && (
                <div style={{ background:"#3d1a1a", border:"1px solid #c94a4a", borderRadius:8, padding:"6px 14px", fontSize:13, color:"#ff8a8a" }}>
                  &#9888; {urgentCount} deadline{urgentCount > 1 ? "s" : ""} within 14 days
                </div>
              )}
              <button onClick={() => { setForm(emptyForm); setEditingId(null); setShowForm(true); }}
                style={{ background:"#1a4a8a", border:"1px solid #2a6ac0", borderRadius:8, color:"#a8d0ff", padding:"8px 20px", cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>
                + Add Opportunity
              </button>
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:12, marginTop:20, flexWrap:"wrap", alignItems:"center" }}>
            <input placeholder="Search grants..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...sel, width:180 }} />
            <select value={filterAgency} onChange={e => setFilterAgency(e.target.value)} style={sel}>
            ...
            <select value={windowMonths} onChange={e => setWindowMonths(Number(e.target.value))} style={sel}>
            ...
            <div style={{ fontSize:11, marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
              {loading                           && <span style={{ color:"#5b8fc9"  }}>Loading&#8230;</span>}
              {!loading && syncStatus==="saving" && <span style={{ color:"#ffd166"  }}>&#8635; Saving&#8230;</span>}
              {!loading && syncStatus==="saved"  && <span style={{ color:"#6aaa64"  }}>&#10003; Saved</span>}
              {!loading && syncStatus==="error"  && <span style={{ color:"#ff6b6b"  }}>&#9888; Save failed</span>}
              {!loading && syncStatus==="idle" && lastUpdated && <span style={{ color:"#4a7a9b" }}>Synced {new Date(lastUpdated).toLocaleTimeString()}</span>}
            </div>
          </div>
        </div>
      </div>
              ...
              {filtered.map(grant => {
                ...
                return (
                  <div key={grant.id} style={{ background:"#111c2e", border:"1px solid #1e3a5f", borderLeft:"4px solid "+ac, borderRadius:12, overflow:"hidden", boxShadow:isExpanded?"0 0 0 1px "+ac+"44":"none" }}>
                    ...
                    {/* Row */}
                    <div onClick={() => setExpandedId(isExpanded ? null : grant.id)}
                      style={{ padding:"18px 22px", cursor:"pointer", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                      ...
                      <div style={{ textAlign:"right", whiteSpace:"nowrap" }}>
                        ...
                        <div style={{ marginTop:3, display:"inline-block", background:urgency.bg, color:urgency.color, borderRadius:5, padding:"2px 10px", fontSize:11 }}>
                          {days<0 ? Math.abs(days)+"d ago" : days===0 ? "TODAY" : days>=9999 ? "—" : days+"d · "+urgency.label}
                        </div>
                      </div>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop:"1px solid #1e3a5f", padding:"18px 22px", background:"#0d1828" }}>
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px 32px", marginBottom:14 }}>
                          {[["Full Agency",grant.agency],["Mechanism",grant.mechanism],["Award Amount",grant.amount],["PI Eligibility",grant.piEligible],["Category",grant.category]]
                            .map(([label,val]) => val ? (
                              <div key={label}>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#111c2e", border:"1px solid #c94a4a", borderRadius:14, padding:"32px 36px", maxWidth:380, textAlign:"center" }}>
            ...
            <div style={{ display:"flex", gap:12, justifyContent:"center" }}>
              <button onClick={() => setConfirmDelete(null)} style={btn("#0d1828","#1e3a5f","#8ab4d4")}>Cancel</button>
              <button onClick={() => handleDelete(confirmDelete)} style={btn("#3d1a1a","#c94a4a","#ff8a8a")}>Remove</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background:"#0f1c30", border:"1px solid #1e3a5f", borderRadius:16, padding:"32px 36px", width:"100%", maxWidth:620, maxHeight:"90vh", overflowY:"auto" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              ...
              <div style={{ gridColumn:"1 / -1" }}>
                ...
                <input value={form.title} onChange={e => setForm(f=>({...f,title:e.target.value}))} placeholder="e.g. NIH R01 – Neural Prosthetics" style={inp()} />
              </div>

              <div>
                <label style={lbl}>Funding Agency *</label>
                <input value={form.agency} onChange={e => setForm(f=>({...f,agency:e.target.value}))} placeholder="e.g. NIH / NIBIB" style={inp()} />
              </div>

              <div>
                <label style={lbl}>Mechanism</label>
                <input value={form.mechanism} onChange={e => setForm(f=>({...f,mechanism:e.target.value}))} placeholder="e.g. R01, CAREER, R21" style={inp()} />
              </div>
              ...
              <div>
                <label style={lbl}>Award Amount</label>
                <input value={form.amount} onChange={e => setForm(f=>({...f,amount:e.target.value}))} placeholder="e.g. $500,000" style={inp()} />
              </div>

              <div>
                <label style={lbl}>PI Eligibility</label>
                <input value={form.piEligible} onChange={e => setForm(f=>({...f,piEligible:e.target.value}))} placeholder="e.g. Early-career faculty" style={inp()} />
              </div>
              ...
              <div style={{ gridColumn:"1 / -1" }}>
                <label style={lbl}>URL / FOA Link</label>
                <input value={form.url} onChange={e => setForm(f=>({...f,url:e.target.value}))} placeholder="https://..." style={inp()} />
              </div>

              <div style={{ gridColumn:"1 / -1" }}>
                <label style={lbl}>Notes</label>
                <textarea value={form.notes} onChange={e => setForm(f=>({...f,notes:e.target.value}))} placeholder="LOI dates, special requirements…" rows={3} style={{ ...inp(), resize:"vertical" }} />
              </div>

            </div>
            <div style={{ display:"flex", gap:12, marginTop:22, justifyContent:"flex-end" }}>
              <button onClick={() => { setShowForm(false); setForm(emptyForm); setEditingId(null); }} style={btn("#0d1828","#1e3a5f","#8ab4d4")}>Cancel</button>
              <button onClick={handleSave} disabled={!form.title||!form.deadline}
                style={btn(form.title&&form.deadline?"#1a4a8a":"#0d1828", form.title&&form.deadline?"#2a6ac0":"#1e3a5f", form.title&&form.deadline?"#a8d0ff":"#4a6a8a")}>
